// ======================================================
// Pizza D'Amico · Apps Script v15
// v14 + HTML-Design-Mails für Contact-Message (inline, kein Drive-Upload nötig)
// ======================================================
...
// ======================================================
// Contact Message — HTML Templates inline
// ======================================================
...
function checkMessageRateLimit(email, phone) {
  try {
    ...
    console.error('msg rate limit check failed:', err);
...
// === Plain-Text Bodies (fallback für mail-clients ohne HTML) ===
function buildMessageOwnerText(d) {
  return [
    '========================================',
...
function buildMessageCustomerText(d) {
  return [
    '========================================',
    "  PIZZA D'AMICO",
    '========================================',
...
// === HTML Bodies (matching Catering-mail style) ===
function htmlEsc(s) {
...
function buildMessageOwnerHtml(d) {
  var name = htmlEsc(d.name);
  var email = htmlEsc(d.email);
  var phone = htmlEsc(d.phone || '—');
  var msg = htmlEsc(d.message);
  var phoneRow = d.phone ? (
    '<tr><td style="padding:6px 0;color:rgba(20,16,12,0.6);vertical-align:top;width:100px;">Telefon</td>' +
    '<td style="padding:6px 0;color:#14100C;"><a href="tel:' + phone.replace(/\s/g, '') + '" style="color:#8E2A1A;text-decoration:none;">' + phone + '</a></td></tr>'
  ) : '';

  return (
    '<!DOCTYPE html><html><body style="margin:0;padding:0;background:#F0E6D2;font-family:Georgia,serif;color:#14100C;">' +
    '<table width="100%" cellpadding="0" cellspacing="0" style="background:#F0E6D2;padding:32px 16px;"><tr><td align="center">' +
    '<table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#FFFEFB;border:1px solid rgba(20,16,12,0.1);border-radius:4px;">' +
    // Header
    '<tr><td style="padding:28px 32px 16px 32px;border-bottom:1px solid rgba(20,16,12,0.08);">' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#C9A66B;font-weight:600;">Neue Nachricht</div>' +
    '<h1 style="margin:10px 0 0 0;font-size:28px;font-weight:600;letter-spacing:-0.5px;color:#14100C;line-height:1.1;">Pizza D&rsquo;Amico</h1>' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:11px;color:rgba(20,16,12,0.55);margin-top:6px;letter-spacing:1.5px;">pizzadamico.ch</div>' +
    '</td></tr>' +
    // Absender
    '<tr><td style="padding:22px 32px 8px 32px;">' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:#C9A66B;font-weight:600;margin-bottom:10px;">Absender</div>' +
    '<table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;line-height:1.55;">' +
    '<tr><td style="padding:6px 0;color:rgba(20,16,12,0.6);vertical-align:top;width:100px;">Name</td>' +
    '<td style="padding:6px 0;color:#14100C;font-weight:600;">' + name + '</td></tr>' +
    '<tr><td style="padding:6px 0;color:rgba(20,16,12,0.6);vertical-align:top;">E-Mail</td>' +
    '<td style="padding:6px 0;color:#14100C;"><a href="mailto:' + email + '" style="color:#8E2A1A;text-decoration:none;">' + email + '</a></td></tr>' +
    phoneRow +
    '</table>' +
    '</td></tr>' +
    // Nachricht
    '<tr><td style="padding:16px 32px 8px 32px;">' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:#C9A66B;font-weight:600;margin-bottom:10px;">Nachricht</div>' +
    '<div style="padding:16px 18px;background:rgba(20,16,12,0.03);border-left:2px solid #C9A66B;border-radius:2px;font-size:14px;line-height:1.6;color:#14100C;">' + msg + '</div>' +
    '</td></tr>' +
    // Footer
    '<tr><td style="padding:20px 32px 28px 32px;border-top:1px solid rgba(20,16,12,0.08);margin-top:12px;">' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:11px;color:rgba(20,16,12,0.55);line-height:1.5;">' +
    'Antworte einfach auf diese Mail &mdash; Reply-To ist der Absender.' +
    '</div>' +
    '</td></tr>' +
...
function buildMessageCustomerHtml(d) {
  var name = htmlEsc(d.name);
  var msg = htmlEsc(d.message);
  ...
  return (
    ...
    // Header
    '<tr><td style="padding:28px 32px 16px 32px;border-bottom:1px solid rgba(20,16,12,0.08);">' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#C9A66B;font-weight:600;">Nachricht erhalten</div>' +
    '<h1 style="margin:10px 0 0 0;font-size:28px;font-weight:600;letter-spacing:-0.5px;color:#14100C;line-height:1.1;">Grazie, ' + name + '.</h1>' +
    '</td></tr>' +
    // Intro
    '<tr><td style="padding:24px 32px 8px 32px;">' +
    '<p style="margin:0;font-size:16px;line-height:1.6;color:#14100C;">Deine Nachricht ist bei uns angekommen. Pietro meldet sich pers&ouml;nlich innerhalb von 48 Stunden.</p>' +
    '</td></tr>' +
    // Deine Nachricht
    '<tr><td style="padding:20px 32px 8px 32px;">' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:#C9A66B;font-weight:600;margin-bottom:10px;">Deine Nachricht</div>' +
    '<div style="padding:14px 16px;background:rgba(20,16,12,0.04);border-left:2px solid #C9A66B;border-radius:2px;font-size:14px;line-height:1.6;color:#14100C;font-style:italic;">' + msg + '</div>' +
    '</td></tr>' +
    // Kontakt
    '<tr><td style="padding:28px 32px 8px 32px;border-top:1px solid rgba(20,16,12,0.08);margin-top:16px;">' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:#C9A66B;font-weight:600;margin-bottom:12px;">Kontakt</div>' +
    '<table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;line-height:1.6;">' +
    '<tr><td style="padding:4px 0;color:rgba(20,16,12,0.6);width:90px;">Telefon</td>' +
    '<td style="padding:4px 0;"><a href="tel:+41763313259" style="color:#8E2A1A;text-decoration:none;font-weight:600;">' + OWNER_PHONE + '</a></td></tr>' +
    '<tr><td style="padding:4px 0;color:rgba(20,16,12,0.6);">E-Mail</td>' +
    '<td style="padding:4px 0;"><a href="mailto:' + OWNER_EMAIL + '" style="color:#8E2A1A;text-decoration:none;">' + OWNER_EMAIL + '</a></td></tr>' +
    '<tr><td style="padding:4px 0;color:rgba(20,16,12,0.6);vertical-align:top;">Adresse</td>' +
    '<td style="padding:4px 0;color:#14100C;">Schlossstrasse 15<br>3110 M&uuml;nsingen</td></tr>' +
    '</table>' +
    '</td></tr>' +
    // Footer
    '<tr><td style="padding:24px 32px 28px 32px;">' +
    '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;font-size:10px;letter-spacing:2px;color:rgba(20,16,12,0.4);text-transform:uppercase;text-align:center;">Pizza D&rsquo;Amico &middot; Grazie Mille</div>' +
    '</td></tr>' +
    '</table></td></tr></table></body></html>'
  );
}
...
// ======================================================
// Catering Rate-Limit + Reservation (unchanged from v14)
// ======================================================
...
function customerText(d, reference, b) {
  ...
  lines.push('  Pietro meldet sich persoenlich zur Bestaetigung.');
  lines.push('');
...
// Helpers
function padRight(s, len) { s = String(s); while (s.length < len) s += ' '; return s; }
function repeat(char, n) { var out = ''; for (var i = 0; i < n; i++) out += char; return out; }
